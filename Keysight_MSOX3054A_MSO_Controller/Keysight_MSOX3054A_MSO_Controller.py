...
class Keysight_MSOX3054A_WaveGen_Controller(Keysight_MSOX3054A_MSO_Controller_GUI.frame_Main):
	...
	def __init__(self, parent):
		...
		self.led_Display1.SetValue("000.000")
		...
		self.led_Display2.SetValue("000.000")
		...
		# LED Setup
		self.led_Display3 = gizmos.LEDNumberCtrl(self.panel_LtoN, -1)
		self.led_Display3.SetValue("000.000")
		...
		self.panel_LtoN.SetSizer(sizer13)
		self.panel_LtoN.Fit()
	...
	def mso_receive_message(self, message):
		...
		self.led_Display1.SetValue(f"{counts:6.3f}")
		self.led_Display2.SetValue(f"{ac_rms:4.3f}")
		self.led_Display3.SetValue(f"{n2e_vrms:6.3f}")
		...
		if self.toggleButton_StartLogging.GetValue():
			...
			try:
				with open(self.current_log_file, mode='w', newline='') as file:
					...
					writer.writerow(["TIMESTAMPS", "PMT COUNTS(c/s)","AC RMS (V)","AC Frequency (Hz)","AC VPP", "Neutral to Earth Voltage (V)", "HT_Voltage", "TEMPERATURE_C"])
				
				# --- FIX 6: Removed timer start. Logging happens in on_data_received ---
				self.toggleButton_StartLogging.SetLabel("Stop logging")
				self.SetStatusText(f"Logging to {filename}...", 0)
			...
			self.SetStatusText("Logging Stopped", 0)
			self.current_log_file = None
	...
	def write_to_csv(self, counts, ac_vpp, ac_rms, ac_freq, n2e_vrms):
		...
		if self.grid_row_counter < 15:
			...
			target_row = 14  # We will write to the last row
	...
	def dirPicker_OnDirChanged(self, event):
		...
		if os.path.exists(selected_path):
			self.SetStatusText(f"Log folder set to: {selected_path}", 0)
		event.Skip()
